...
public class Client {
    ...
    public void setTCPPort2(int TCPPort2) {
        this.TCPPort2 = TCPPort2;
    }
    ...
    public Client(Client client) {
        ...
        this.TCPPort2 = client.getTCPPort2();
    ...
    public static Client parseMessage(String message) {
        ...
        if (parts.length >= 6) {
            ...
            int TCPPort2 = Integer.parseInt(parts[3]); // 取得第二個 TCP 埠號
            int UDPPort = Integer.parseInt(parts[4]); // 取得 UDP 埠號
            String OS = parts[5]; // 取得作業系統名稱
            return new Client(IPAddr, userName, TCPPort, TCPPort2, UDPPort, OS); // 返回新的 Client 物件
        } else {
