...
public class Main { // 定義 Main 類別
    ...
    static Client client = new Client(); // 建立 Client 物件
    public static void initClient() {
        // 靜態初始化區塊，用以設定端口號
        client.setTCPPort(getFreeTCPPort()); // 取得可用的 TCP 端口
        client.setUDPPort(UDP_PORT_Manager.getFreeUDPPort()); // 取得可用的 UDP 端口
        try { // 嘗試取得本機主機名稱
            client.setUserName(InetAddress.getLocalHost().getHostName()); // 取得主機名稱並指定給 USER_NAME
        } catch (UnknownHostException e) { // 異常處理：未知主機
            client.setUserName(System.getProperty("user"+UUID.randomUUID().toString().substring(0, 8))); // 使用隨機字串作為使用者名稱
        }
        client.setOS(System.getProperty("os.name")); // 取得作業系統名稱並指定給 OS
        client.setIPAddr(getNonLoopbcakIP()); // 取得並設定本機的非迴圈 IP 位址        )
    ...
    public void sendACK(Socket socket) { // 定義傳送 ACK 訊息的方法
        try { // 嘗試傳送 ACK
    ...
    public static void main(String[] args) { // 主方法，程式入口點
        ...
        System.out.println("IP 地址: " + client.getIPAddr() + " TCP端口號: " + client.getTCPPort() + " UDP端口號: " + client.getUDPPort() + " 使用者名稱: " + client.getUserName() + " 作業系統: " + client.getOS()); // 輸出系統資訊
        new Thread(() -> new UDPServer().Server(client)).start(); // 建立新執行緒並啟動 UDP 伺服器
    ...
    public void receiveFile() { // 定義接收檔案的方法
        try { // 嘗試建立 TCP 伺服器以接收連線
            ...
            while (true) { // 無限迴圈等待客戶端連線
                ...
                System.out.println("接收到來自 " + socket.getInetAddress() + " 的連線請求 port" + socket.getPort()); // 輸出連線來源資訊
    ...
    public static boolean sendFileToUser(String selectedUser, File file, Consumer<Integer> progressCallback) {
        Client targetClient = client.clientList.get(selectedUser);
        ...
        try {
            ...
            // Here you could compress the file if desired.
            File originalFile = file;
            Socket socket = new Socket(targetClient.getIPAddr(), targetClient.getTCPPort());
            System.out.println("Starting to send file: " + originalFile.getName() + " to " + targetClient.getIPAddr() + ":" + targetClient.getTCPPort());
            PrintWriter out = new PrintWriter(socket.getOutputStream(), true);
            // Use a delimiter that won’t conflict with file names (e.g., pipe '|')
            out.println(originalFile.getName() + ":" + originalFile.length()); 
            out.flush();
            
            // Wait for initial ACK from receiver
            short cnt = 0;
            ...
            int bytesRead;
            long totalBytes = originalFile.length();
            long bytesSent = 0;
            ...
            while ((bytesRead = fis.read(buffer)) != -1) {
                ...
                os.flush();
                bytesSent += bytesRead;
                // Wait for ACK after sending the chunk
                if (recevieACK(socket)) {
    ...
    public void printHashTable(Hashtable<String, Client> clientLists) { // 定義列印客戶端哈希表的方法
        System.out.println("目前有 " + clientLists.size() + " 個已連線的客戶端: "); // 輸出已連線的客戶端數量
        for (String key : clientLists.keySet()) { // 遍歷客戶端哈希表
            System.out.println("使用者名稱: " + key + ", 端口號: " + clientLists.get(key).getTCPPort() + ", 作業系統: " + clientLists.get(key).getOS()); // 輸出每個客戶端的詳細資訊
        }
    }
